...
public class Ventana extends JFrame{
	...
	public void calculadora() {
	    ...
	    */
		...
		panelP.setBackground(Color.decode("##6d86e1"));
		
		JLabel input = new JLabel("200",4);
		input.setOpaque(true);
		input.setBackground(Color.WHITE);
		input.setFont( new Font("Arial", Font.BOLD, 40) );
		panelP.add(input, BorderLayout.NORTH);
		
		JPanel actions = new JPanel();
		actions.setBackground(Color.WHITE);
		actions.setLayout(new GridLayout(4,3,10,10));
		panelP.add(actions,BorderLayout.CENTER);
		
		String num[] = {"7","8","9","6","5","4","3","2","1","0"};
		
		for (int i = 0; i < num.length; i++) {
			 
			JButton numBtn = new JButton(num[i]);
			numBtn.setSize(100, 100);
			actions.add(numBtn);
		}
		
		JPanel operacion = new JPanel();
		
		operacion.setLayout(new GridLayout(3,1,20,20));
		panelP.add(operacion,BorderLayout.EAST);
		
		String ope[] = {"+","-","=",".","/","CE"};
		
		for (int i = 0; i < ope.length; i++) {
			 
			JButton opeBtn = new JButton(ope[i]);
			opeBtn.setSize(100, 100);
			operacion.add(opeBtn);
		}
